...
def tokenizer(s, ngram_range = (1,3)):
...
def create_vectors(X_train, X_test, analyzer = tokenizer):
...
score = list()
slider = np.linspace(0,2.5,10).tolist()
type(slider)
for i in slider:
...
plt.axvline(x=0.277778, color='k', linestyle='--')
plt.text(0.3,0.55,'Optimal Sigma = 0.278',rotation=0)
plt.show()
p.savefig('180601_ROC_NBayes_00.pdf', format='pdf', dpi=400)
...
false_pos, false_neg = get_errors(V_test, y_test, preds)
